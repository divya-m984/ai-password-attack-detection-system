...
**This document covers the artifact contract, not the result.** Models are
fitted, calibrated, thresholded, published as immutable runs, recorded in an
append-only ledger, compared against predeclared gates on validation-B, and
frozen as a champion. The locked TEST evaluation, the fusion selection, and the
experimental novel-anomaly holdout evaluation are a separate contract with its
own document — see [docs/test-evaluation.md](test-evaluation.md) — and **every
figure it produces was measured on synthetic traffic generated by this
repository**, which is not evidence about real authentication systems.
...
Selection logic itself is a separate contract; nothing in this section
implements it. See [docs/champion-selection.md](champion-selection.md).
...
- **No ledger record is written by prediction.** Prediction identity lives on
  the `PredictionManifest`. The `test_evaluation` record type is written by the
  locked evaluation alone.

---

## 22. Known limitations

**Nothing in this document is a result.** It describes what an artifact must
carry and guarantee. The locked TEST evaluation, the fusion selection, the
rule/model/hybrid comparison, deterministic attribution, and drift monitoring
each have their own contract and their own document:
[docs/test-evaluation.md](test-evaluation.md),
[docs/explainability.md](explainability.md), and
[docs/drift-monitoring.md](drift-monitoring.md).

**Every measured figure is synthetic.** Whatever the locked evaluation reports
was measured on traffic this repository generated under a declared scenario
configuration. It is evidence that the pipeline behaves as specified. It is not
evidence of real-world efficacy, and no number should be quoted as one.
